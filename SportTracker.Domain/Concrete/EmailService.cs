...
using SportTracker.Domain.Resources;
...
namespace SportTracker.Domain.Concrete
{
	public class EmailService : IEmailService
	{
		...
		public EmailService(SmtpClient smtpClient)
		{
			...
			smtpClient.Credentials = new NetworkCredential("groshevoy@gmail.com", "139174");
			smtpClient.Host = " smtpcorp.com";
		...
		void IEmailService.SendEmail(string emailAddress, string subject, string message)
		{
			...
			using (var mailMessage = new MailMessage())
			{
				...
				mailMessage.From = new MailAddress("niceboy@nm.ru");
		...
		void IEmailService.SendEmail(SendEmailModel sendEmailModel, string templateName, object data)
		{
			...
			using (var stringWriter = new StringWriter())
			{
				...
				var newHttpContext = new HttpContext(new HttpRequest("/", sendEmailModel.WebsiteURL, ""), httpResponse)
					{
				...
				controllerContext.RequestContext = new RequestContext(new HttpContextWrapper(newHttpContext), new RouteData());
				controllerContext.RouteData.Values.Add("controller", "foo");

				ViewEngineResult viewResult = ViewEngines.Engines.FindPartialView(controllerContext, String.Format("~/Views/EmailTemplates/{0}.cshtml", templateName));
				ViewContext viewContext = new ViewContext(controllerContext, viewResult.View, viewData, tempData, httpResponse.Output);
